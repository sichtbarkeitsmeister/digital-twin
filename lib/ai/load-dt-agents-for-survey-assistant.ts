...
export async function loadDtAgentsForSurveyAssistant(input: {
  ...
  let query = input.supabase
    .from("dt_agents")
    .select(
      "id,organisation_id,name,role,slug,kind,uses_global_prompt,prompt_template,prompt_append",
    )
  ...
  const focusedAgentPrompts: FocusedDtAgentPrompt[] = agents
    .filter((a) => focusedIds.has(a.id))
    .slice(0, 3)
    .map((a) => ({
      id: a.id,
      name: a.name,
  ...
  return { knownAgents, focusedAgentPrompts };
}
