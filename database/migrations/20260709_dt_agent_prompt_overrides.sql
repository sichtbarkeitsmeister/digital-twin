-- Per-org prompt overrides: prompt_append + toggle uses_global_prompt
-- Archive geo_advisor and wunschkunde templates (existing instances remain).

ALTER TABLE public.dt_agents
  ADD COLUMN IF NOT EXISTS prompt_append text;

-- ---------------------------------------------------------------------------
-- Update agent: allow toggling global sync, custom prompt when unsynced, append
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  SET
    ...
    prompt_template = CASE
      WHEN v_new_uses_global THEN a.prompt_template
...
-- ---------------------------------------------------------------------------
-- Archive removed marketplace templates (keep existing agent instances)
-- ---------------------------------------------------------------------------
UPDATE public.dt_agent_templates
SET archived_at = timezone('utc'::text, now()),
    is_public = false
WHERE slug IN ('geo_advisor', 'wunschkunde')
