-- Auto-fill organisations.slug from name when omitted; backfill existing NULLs.
-- Slug is required for SEO/n8n client routing (loadSeoReportContext).
...
-- Backfill existing orgs with empty slug (deterministic; collision → -2, -3, …).
DO $$
DECLARE
  r record;
  base text;
...
BEGIN
  ...
  LOOP
    base := public.slugify_organisation_name(r.name);
    IF base IS NULL THEN
      CONTINUE;
    END IF;
    candidate := public.allocate_unique_organisation_slug(base);
