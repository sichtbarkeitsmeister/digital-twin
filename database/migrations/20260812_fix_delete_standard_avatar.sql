-- Re-assert: only SEO-Berater is undeletable. Legacy Standard-Avatar (slug=default)
-- and other non-SEO agents may be removed. Idempotent CREATE OR REPLACE.
...
BEGIN
  ...
  SELECT a.organisation_id, a.slug, a.kind INTO v_org, v_slug, v_kind
  ...
  -- Only the SEO advisor stays protected (not every is_default row).
  IF v_slug = 'seo_advisor' OR v_kind = 'seo_advisor' THEN
    RAISE EXCEPTION 'seo_advisor_protected';
  END IF;
