-- SEO report: optional "send to organisation owner" flag.
-- Adds tracking columns and extends dt_queue_seo_report with a send-to-owner arg.

ALTER TABLE public.dt_seo_reports
  ADD COLUMN IF NOT EXISTS send_to_owner boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS owner_sent_at timestamptz;

-- Replace the 2-arg queue function with a 3-arg version (3rd arg defaulted so
-- existing 2-arg callers keep working).
DROP FUNCTION IF EXISTS public.dt_queue_seo_report(uuid, text);

CREATE OR REPLACE FUNCTION public.dt_queue_seo_report(
  p_organisation_id uuid,
  p_recipient_type text,
  p_send_to_owner boolean DEFAULT false
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  VALUES (
    ...
    p_recipient_type,
    v_email,
    COALESCE(p_send_to_owner, false),
    'queued',
    v_cfg.website_url,
    v_cfg.focus_keyword,
...
GRANT EXECUTE ON FUNCTION public.dt_queue_seo_report(uuid, text, boolean) TO authenticated;
